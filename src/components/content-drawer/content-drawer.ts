...
export class ContentDrawerComponent {
    ...
    ngAfterViewInit() {
  	...
  	if (!this.options.hideTitle) {
        this.options.hideTitle = this.hideTitle;
      }

  	if (!this.options.footer) {
        this.options.footer = this.footer;
      }
   ...
   */
    ...
    /*
     handleTap(ev) {
      ...
      } else if (((this.platform.height() - newTop) < this.thresholdBottom && ev.additionalEvent === "pandown") || bounceToTop) {
        this.domCtrl.write(() => {
          this.renderer.setElementStyle(this.element.nativeElement, 'transition', 'top 0.5s');
